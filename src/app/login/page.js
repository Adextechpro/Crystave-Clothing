...
export default function LoginPage() {
  // States to manage what the user sees
  ...
  const handleSubmission = (e, nextStep) => {
    e.preventDefault();
    setIsLoading(true);

    // Simulate a 2-second "Atelier" processing delay
    setTimeout(() => {
      setIsLoading(false);
      if (nextStep === "sent") {
        setEmailSent(true);
  ...
  return (
    <main className="min-h-screen flex flex-col md:flex-row bg-[#FAF9F6]">
      {/* LEFT SIDE: Brand Visual (Constant) */}
        ...
        <div className="w-full max-w-md">
          ...
          {view === "login" && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
              ...
              <form onSubmit={(e) => handleSubmission(e, "login")} className="space-y-8">
                <input type="email" placeholder="EMAIL ADDRESS" required className="w-full bg-transparent border-b border-[#EADDCA] py-3 text-sm outline-none focus:border-[#4A4238] transition-colors text-[#4A4238]" />
                ...
                <div className="flex justify-between items-center text-[10px] tracking-widest uppercase text-[#918170]">
                  ...
                  {/* IMPORTANT: This button switches the view */}
                  <button type="button" onClick={() => setView("forgot")} className="hover:text-[#4A4238] transition-colors">
                    Forgot Password?
                  </button>
                </div>
          ...
          {view === "forgot" && !emailSent && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
              ...
              <form onSubmit={(e) => handleSubmission(e, "sent")} className="space-y-8">
          ...
          {/* Create Account Link (Hidden during loading) */}
          {!isLoading && !emailSent && (
